...
const games = new Map();
const quests = [
 {title:"Resource Hunt",icon:"🌲"},{title:"Zombie Attack",icon:"🧟"},{title:"Diamond Hunt",icon:"💎"},{title:"Crafting – Diamond Sword",icon:"⚔️"},{title:"Creeper Explosion",icon:"💥"},{title:"Survive the Night",icon:"🏰"},{title:"Spider Cave",icon:"🕷️"},{title:"Nether Portal",icon:"🔥"},{title:"The Final Teamwork",icon:"🤝"},{title:"Diamond Chest",icon:"🎁"}
];
...
function startQuest(t,q){t.q=q;t.done=false;t.approved=false;t.timerStart=q===4?Date.now()+7000:null;t.codeFragment=q===8?(t._fragment||""):""}
...
function advance(game){const c=game.teams.creeper,d=game.teams.diamond;if(c.q!==d.q||!c.approved||!d.approved||c.q>=quests.length-1)return false;const nq=c.q+1;startQuest(c,nq);startQuest(d,nq);if(nq===8){c.codeFragment=c._fragment;d.codeFragment=d._fragment}broadcastForGame(game);return true}
...
wss.on("connection",ws=>{
 ws.on("message",raw=>{
  ...
  if(m.type==="auto_done"&&(m.role==="creeper"||m.role==="diamond")){const t=game.teams[m.role];if(t.q===3)t.done=true;broadcast(code);return}
  if(m.type==="timer_done"&&(m.role==="creeper"||m.role==="diamond")){const t=game.teams[m.role];if(t.q===4)t.done=true;broadcast(code);return}
  if(m.type==="portal_done"&&(m.role==="creeper"||m.role==="diamond")){const t=game.teams[m.role];if(t.q===7)t.done=true;broadcast(code);return}
  if(m.type==="teamwork_done"&&(m.role==="creeper"||m.role==="diamond")){const t=game.teams[m.role];if(t.q===8)t.done=true;broadcast(code);return}
